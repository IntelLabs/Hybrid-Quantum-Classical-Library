...
// Y1*X1 = -iZ1
// (X0 Y1)*(Y0 X1) = -(-1) Z0 Z1
Vec2DMat PauliOp::qubitwiseCommutation() {
  Vec2DMat vmat =
      Vec2DMat(this->op_sum.size(), vector<int>(this->op_sum.size(), 0));
  int row = 0, col = 0;
  for (auto iIter = this->op_sum.begin(); iIter != this->op_sum.end();
       ++iIter, ++row) {
    for (auto jIter = next(iIter); jIter != this->op_sum.end();
         ++jIter, ++col) {
      pstring::const_iterator loc_i_begin = iIter->first.begin();
      pstring::const_iterator loc_i_end = iIter->first.end();
      pstring::const_iterator loc_j_begin = jIter->first.begin();
      pstring::const_iterator loc_j_end = jIter->first.end();

      for (pstring::const_iterator &ipstrIter = loc_i_begin;
           ipstrIter != loc_i_end && loc_j_begin != loc_j_end; ++ipstrIter) {
        while (loc_j_begin != loc_j_end) {
          int a = loc_i_begin->first;
          int b = loc_j_begin->first;
          char U = loc_i_begin->second;
          char V = loc_j_begin->second;
          if (U == 'X' && V == 'Y' &&
              (a == b)) { // Add edge for non-commutation
            vmat[row][col] = 1;
          } else if (U == 'Y' && V == 'X' && (a == b)) {
            vmat[row][col] = 1;
          } else if (U == 'X' && V == 'Z' && (a == b)) {
