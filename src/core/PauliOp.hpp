...
class PauliOp {
    ...
    to do:
    - get the sting addTerm to work. (do *not* do multi-term)
    ...
    public:
        ...
        bool qubitwiseCommutation(std::vector<std::string>& pauliStrings);
        op_pair processLocCharString( std::string inp );
        std::string getCharString();

        PauliOp() = default;
        PauliOp(const PauliOp&) = default;
        PauliOp& operator=(const PauliOp& P) { this->op_sum = P.op_sum; return *this; }
