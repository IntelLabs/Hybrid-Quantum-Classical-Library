...
// Update : Comments added throughout to clarify the intentions behind code blocks (08/27/2021)
...
/* alpha1, alpha2, gamma1, gamma2 is the order of the variational angles used in here */
quantum_shared_double_array QuantumVariableParams[N];

static int steps_count = 0;

/* 
When using the Intel Quantum Simulator (IQS) at the lowest level, it is necessary to 
decide how to calculate the cost function based on the results returned. If using the full state
vector, then only one experiment is required per iteration. 

In this script, we will be using the probabilities corresponding to each of the basis states (e.g. 0000, 1010)
These are returned simply by calculating the probability corresponding to each of the state vector element
and stored in the special array ProbabilityRegister.
*/
...
with the appropriate decompositions such that the Intel Quantum Compiler can handle them.
*/
...
Almost identical to the previous quantum kernel tfdQ4_Z, in this qkernel, four Hadamard operations are inserted
just prior to measurement of the qubits. This enables mapping from X-basis to Z-basis, so that X related 
...
/* 
The three expectation values of the observables have been written as functions below. The relationships between
the expectation values and the probabilities obtained from the state vector amplitudes was derived independently 
with simple matrix calculations of Pauli matrix products.
*/
double ZZApZZB_expectation (std::vector<double> ProbReg) {
  auto line0 = ProbReg[0] + ProbReg[3] - ProbReg[5] - ProbReg[6] - ProbReg[9] - ProbReg[10] + ProbReg[12] + ProbReg[15];
  return 2 * line0;
}
...
double ZApZB_expectation (std::vector<double> ProbReg) {
  ...
  auto line1 = ProbReg[1] + ProbReg[2] + ProbReg[4] - ProbReg[7] + ProbReg[8] - ProbReg[11] - ProbReg[13] - ProbReg[14];
  return (4 * line0 + 2 * line1);
}

/*
The total cost function is constructed here using the expectation values from before. There is freedom in the numeric
coefficient and index used. Here we use the best engineered simple cost function from Ref. 3
*/
double total_cost (double inv_temp, std::vector<double> ProbReg_Z, std::vector<double> ProbReg_X) {
...
double ansatz_run_lambda (const arma::mat &m, double inv_temp) {
    // loading the new variational angles into the special global array for hybrid compilation
    QuantumVariableParams[0] = m(0); 
    QuantumVariableParams[1] = m(1);
    QuantumVariableParams[2] = m(2);
    QuantumVariableParams[3] = m(3);
    
    // two local variables to store the results from the two different experiments run in each iteration
...
int main()
{
    ...
    // Clearing the file contents used to store all the results from TFD minimization
    std::ofstream file_to_clear;
    file_to_clear.open("tfd_minimization_results.csv", std::ofstream::out | std::ofstream::trunc);
    file_to_clear.close();
